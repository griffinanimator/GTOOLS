...
class Leg_Joint_Utils:
    def __init__(self):
        """ This is the dictionary """
        self.jointList = {}

    def leg_createJoints(self, jointNames, jointPos):
        ...
        for index in range (len(jointNames)):
            ...
            cmds.setAttr(bnJnt + ".rotateOrder", 1)
    ...
    def leg_createFKJoints (self, jointNames, jointPos):
        ...
        for index in range (len(jointNames)):
            ...
            cmds.setAttr(fkJnt + ".rotateOrder", 1)
            
        self.jointList['fk'] = fkJoints
        return (fkJoints)

    def leg_createIKJoints (self, jointNames, jointPos):
        """ Clearing my selection just in case """
        cmds.select(d=True)
        ...
        for index in range (len(jointNames)):
            ...
            cmds.setAttr(ikJnt + ".rotateOrder", 1)
    ...
    def groupLegs (self, jointNames, jointPos):
        """Group the BN Leg, FK Leg, and IK Leg chains together"""
        GrpLeg = cmds.group("BN_"+jointNames[0], "FK_"+jointNames[0], "IK_"+jointNames[0], name='JNT_Leg_GRP')
        cmds.setAttr(GrpLeg + ".rotateOrder", 1)
